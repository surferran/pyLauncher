...
'''
purpose:
to enable web(javascript etc.) framework for reading csv and displaying graphs
'''
...
class DataTable(gridlib.PyGridTableBase):
    ...
    def GetValue(self, row, col):
        ...
        return str(self.data.ix[row, col-1])
        #################################        #################################
...
def display(df):
    app = wx.App()
    frame = PandasTable(None, 'pandas grid test', df)
    app.MainLoop()
# ###################################################################
def main():
    import pandas as pd
...
if __name__ == '__main__':
    print "self test run , for 'pandasgrid.py' "
    main()
